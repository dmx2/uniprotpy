...
- Version 0.2 packaging and deployment: complete. Poetry metadata and duplicate dependency files are replaced by PEP 621, Hatchling, and one uv lock; pandas was removed after confirming no runtime use. Slim CI verifies Python 3.10 and 3.14. A published GitHub Release must match the package tag (`v0.2`), reruns 123 offline tests, checks and smoke-tests the wheel/source distribution, and exposes `PYPI_API_TOKEN` only to the final token-authenticated PyPI upload step.

## Next step

Operational release step: add the `PYPI_API_TOKEN` repository secret, then publish GitHub Release `v0.2` from tag `v0.2`; `.github/workflows/publish.yml` performs the verified PyPI upload. No further implementation milestone is committed. Future UniParc persistence/CLI work or taxonomy and ID Mapping CLI/storage work should be scoped explicitly and preserve raw documents, response metadata, opaque cursors, source provenance, mapping diagnostics, and the shipped client/domain/release boundaries.
